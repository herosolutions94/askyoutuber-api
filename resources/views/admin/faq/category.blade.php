    ...
    @if (request()->segment(3) == 'edit' || request()->segment(3) == 'add')
        <div class="intro-y flex items-center mt-8">
            <h2 class="text-lg font-medium mr-auto">
                {{ request()->segment(3) == 'edit' ? 'Edit' : 'Add New' }} Category
            </h2>
        </div>
        @if (session('success'))
                        ...
                        <div class="grid grid-cols-12 gap-2 mt-3">
                            ...
                            <div class="col-span-12 xl:col-span-6">
                                <div class="mt-3">
                                    ...
                                    <select class="select2 w-full" name="parent">
                                        <option value="0">Nor Selected</option>
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                {{ !empty($row) ? ($row->parent == $category->id ? 'selected' : '') : '' }}>
    ...
    @else
        ...
        <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
            ...
            <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
                <a class="button text-white bg-theme-1 shadow-md mr-2" href="{{ url('admin/faq_categories/add') }}">Add New
                    FAQ Category</a>
            </div>
        </div>
            ...
            <table class="table table-report table-report--bordered display datatable w-full">
                <thead>
                    <tr>
                        ...
                        <th class="border-b-2 text-center whitespace-no-wrap">PARENT CATEGORY</th>
                    ...
                    @if (!empty($rows))
                        @foreach ($rows as $key => $row)
                            <tr class="odd">
                                ...
                                <td class="w-40 border-b">{!! get_faq_category_name($row->parent) !!}</td>
                                <td class="w-40 border-b">{!! getStatus($row->status) !!}</td>
                                <td class="border-b w-5">
                                    <div class="flex sm:justify-center items-center">
                                        <a class="flex items-center mr-3"
                                            href="{{ url('admin/faq_categories/edit/' . $row->id) }}"> <i
                                                data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
                                        <a class="flex items-center text-theme-6"
                                            href="{{ url('admin/faq_categories/delete/' . $row->id) }}"
