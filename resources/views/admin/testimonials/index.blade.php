    ...
    @if (request()->segment(3) == 'edit' || request()->segment(3) == 'add')
        <div class="intro-y flex items-center mt-8">
            <h2 class="text-lg font-medium mr-auto">
                {{ request()->segment(3) == 'edit' ? 'Edit' : 'Add New' }} Testimonial
            </h2>
        </div>
        @if (session('success'))
                ...
                <form class="form theme-form" method="post" action="" enctype="multipart/form-data">
                    ...
                    <div class="intro-y box p-5">
                                ...
                                <div class="border border-gray-200 rounded-md p-5 change_thumbnail_image">
                                    <div class="w-40 h-40 relative image-fit cursor-pointer zoom-in mx-auto">
                                        <img class="rounded-md" alt=""
                                            src="{{ get_site_image_src('testimonials', !empty($row) ? $row->image : '') }}">
                                    </div>
                        ...
                        <div class="mt-3">
                            ...
                            <input type="text" class="input w-full border mt-2"
                                value="{{ !empty($row) ? $row->address : '' }}" name="address" required>
                        </div>
                        <div class="mt-3">
                            <label>Text Block</label>
                            <textarea data-feature="all" class="ckeditor" name="message">{{ !empty($row) ? $row->message : '' }}</textarea>
                        </div>
                        <div class="mt-3">
    ...
    @else
        ...
        <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
            ...
            <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
                <a class="button text-white bg-theme-1 shadow-md mr-2" href="{{ url('admin/testimonials/add') }}">Add New
                    Testimonial</a>
            </div>
        </div>
                                ...
                                <td class="border-b w-5">
                                    <div class="flex sm:justify-center items-center">
                                        <a class="flex items-center mr-3"
                                            href="{{ url('admin/testimonials/edit/' . $row->id) }}"> <i
